...
export function Hero({ locale, eyebrow, title, description, heroCard, showResearchActions = false }: HeroProps) {
  return (
    <section className="relative isolate overflow-hidden bg-slate-950 text-white">
      ...
      />
      <div className="absolute inset-0 -z-10 bg-slate-950/75" aria-hidden="true" />
      <div className={`mx-auto grid min-h-[560px] max-w-[1200px] gap-8 px-4 py-12 sm:px-6 lg:px-8 lg:py-16 ${showResearchActions ? 'items-center' : 'items-end lg:grid-cols-[1fr_340px]'}`}>
        <div className="max-w-3xl">
          <p className="mb-4 font-black uppercase tracking-[0.2em] text-amber-300">{eyebrow}</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {researchActions.map(([label, href], index) => (
                <a
                  ...
                  className={index === 0
                    ? 'border-2 border-amber-300 bg-amber-300 px-5 py-3 text-sm font-black uppercase tracking-wide text-slate-950'
                    : 'border-2 border-white bg-slate-950/60 px-5 py-3 text-sm font-black uppercase tracking-wide text-white'}
                >
                  {label}
                </a>
              ))}
        ...
        {!showResearchActions && (
          <aside className="border-4 border-slate-950 bg-amber-300 p-6 text-slate-950 shadow-[8px_8px_0_0_#f97316]">
