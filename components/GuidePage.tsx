...
export function GuidePage({ page }: { page: PageDocument }) {
  ...
  const reservesFaqTocItem = bodyH2Count < frontmatter.toc.length;
  const navigationToc = reservesFaqTocItem
    ? frontmatter.toc.map((item, index) => index === frontmatter.toc.length - 1 ? { ...item, id: `${faqId}-list` } : item)
    : frontmatter.toc;
